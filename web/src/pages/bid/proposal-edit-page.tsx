...
const MOCK_OPINION =
  "모던 건축을 기반으로 자연 채광과 통풍을 극대화한 설계를 제안합니다. 남향 배치를 통해 일조량을 확보하고, 중정을 활용하여 내부 공간에 자연광이 깊숙이 들어오도록 계획했습니다. 외부 마감재는 노출콘크리트와 목재를 조합하여 모던하면서도 따뜻한 느낌을 연출하고자 합니다. 에너지 효율을 고려한 단열 설계와 친환경 자재 사용을 기본으로 합니다.";

const MOCK_FILES: UploadedFile[] = [
  { name: "설계안_초안.pdf", size: "2.4MB" },
];

/* ─── Page ─── */

export default function ProposalEditPage() {
  const { bidId, proposalId: _proposalId } = useParams();
  void _proposalId;

  // Form state (pre-filled)
  const [fee, setFee] = useState("4200");
  const [opinion, setOpinion] = useState(MOCK_OPINION);
  ...
  return (
    <div className="min-h-svh bg-background">
      ...
      <div className="mx-auto max-w-[760px] px-6 py-8">
        ...
        <h1 className="mb-2 text-2xl font-bold tracking-tight">입찰 제안서 수정</h1>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <Button className="rounded-xl h-12" onClick={() => alert("저장 완료 (목업)")}>
            저장
          </Button>
        </div>
      </div>
